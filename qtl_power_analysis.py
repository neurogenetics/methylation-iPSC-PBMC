...
def compute_empirical_alpha(file_path):
    print(f"Reading QTL results from: {file_path} to compute alpha...")
    try:
        # Read only the necessary columns to save memory and keep execution fast
        df = pd.read_csv(file_path, sep="\t", usecols=["pval_nominal", "qval"])
    except Exception as e:
        raise IOError(f"Failed to read/parse QTL result file '{file_path}': {e}")
        
    sig_df = df[df["qval"] <= 0.05]
    if sig_df.empty:
        raise ValueError(f"No significant associations (qval <= 0.05) found in {file_path} to compute alpha.")
    
    alpha = sig_df["pval_nominal"].max()
...
def solve_min_detectable_r_squared(n, alpha, target_power, power_analysis):
    ...
    return r_squared * 100 # return as percentage (e.g. 29.27)
...
def process_dataset(label, n, results_path, var_threshold, alpha, target_power):
    ...
    print(f"Power-derived {int(target_power * 100)}% R2 Threshold: {var_threshold:.2f}%")
    print("="*65)
    
    # 1. Load results file
    print(f"Reading QTL results from: {results_path}...")
    try:
        df = pd.read_csv(
            results_path,
            sep="\t",
            usecols=["slope", "slope_se", "pval_nominal", "qval"]
        )
    except Exception as e:
        raise IOError(f"Failed to read/parse QTL result file '{results_path}': {e}")
    ...
    print("\nTop 5 QTL by Variance Explained:")
    top_5 = df.sort_values("Variance_Explained_Pct", ascending=False).head()
    print(top_5[["slope", "slope_se", "pval_nominal", "qval", "Variance_Explained_Pct"]].to_string(index=False))
    
    # Print bottom 5 significant ones by variance explained
    sig_df = df[df["qval"] <= 0.05]
    print("\nBottom 5 Significant QTL (qval <= 0.05) by Variance Explained:")
    bottom_5 = sig_df.sort_values("Variance_Explained_Pct", ascending=True).head()
    print(bottom_5[["slope", "slope_se", "pval_nominal", "qval", "Variance_Explained_Pct"]].to_string(index=False))
    ...
    print("\n" + "-"*50)
    print(f"Empirical Threshold from {int(target_power * 100)}% Power: {var_threshold:.2f}%")
    print(f"Total Significant meQTL (qval <= 0.05): {total_sig:,}")
    print(f"Significant meQTL >= Threshold: {count_above:,} ({pct_above:.2f}%)")
    print("-"*50)
    ...
    df_sorted = df.sort_values("Variance_Explained_Pct", ascending=True).reset_index(drop=True)
    
    # Separate into significant and non-significant for plotting
    df_sig = df_sorted[df_sorted["qval"] <= 0.05]
    df_nonsig = df_sorted[df_sorted["qval"] > 0.05]
    ...
    if len(x_sig) > 0:
        ax.scatter(
            ...
            s=3,
            alpha=0.7,
            label="Significant meQTL (qval <= 0.05)",
            edgecolors="none"
        )
    ...
    ax.set_title(
        f"Distribution of Variance Explained ($R^2$) across meQTL ({label}, N={n})",
        fontsize=14,
        fontweight="bold",
        pad=15,
    )
    ...
    plt.tight_layout()
    dist_fig_path = f"/mnt/labshare/raph/datasets/adrd_neuro/ipsc_dnam/figures/{label}_variance_explained_distribution.png"
...
def main():
    parser = argparse.ArgumentParser(
        description="Unified tool for calculating methQTL statistical power and comparing with empirical variance explained."
    )
    parser.add_argument(
        "--labels",
        default="iPSC,PBMC",
        help="Comma-separated dataset labels (default: 'iPSC,PBMC')"
    )
    parser.add_argument(
        "--sizes",
        default="87,67",
        help="Comma-separated dataset sample sizes (default: '87,67')"
    )
    parser.add_argument(
        "--results",
        default="/mnt/labshare/raph/datasets/adrd_neuro/ipsc_dnam/methQTL/IPSC.cis_qtl.txt,/mnt/labshare/raph/datasets/adrd_neuro/ipsc_dnam/methQTL/PBMC.cis_qtl_with_celltypes.txt",
        help="Comma-separated paths to TensorQTL cis_qtl.txt result files"
    )
    parser.add_argument(
        "--target-power",
        type=float,
    ...
    parser.add_argument(
        "--output",
        default="/mnt/labshare/raph/datasets/adrd_neuro/ipsc_dnam/figures/methQTL_Power_Curve.png",
        help="Path to save the generated theoretical power curves comparison plot (default: 'methQTL_Power_Curve.png')"
    )
    ...
    print("\n" + "="*65)
    print(f"Step 2: Solving for minimum detectable R^2 at {int(target_power * 100)}% power...")
    print("="*65)
    var_thresholds = []
    for label, n, alpha in zip(labels, sizes, alphas):
        min_r2_pct = solve_min_detectable_r_squared(n, alpha, target_power, power_analysis)
    ...
    print("\n" + "="*65)
    print("Step 3: Processing empirical dataset results using power-solved thresholds...")
    print("="*65)
    for label, n, path, threshold, alpha in zip(labels, sizes, results_paths, var_thresholds, alphas):
    ...
    ax.set_title(
        "Power to Detect methQTL by Variance Explained ($R^2$)", 
        fontsize=14, 
        fontweight="bold", 
        pad=15
    )
    ax.set_xlabel("Variance Explained in Methylation ($R^2$) (%)", fontsize=12, labelpad=10)
